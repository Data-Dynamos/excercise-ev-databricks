...
# MAGIC | --- | --- | --- | -- |
# MAGIC | StartTransaction | Request | Event sent when a Transaction that has been initiated by the car (or by itself on a scheduled basis). This payload contains the start timestamp (`timestamp`) of the charge and the meter reading (`meter_start`) at the time of the event. This does not contain a transaction ID (but the response back to the Charge Point does). | [example json](https://github.com/Data-Dynamos/excercise-ev-databricks/blob/master/sample-data/StartTransactionRequest.json)
# MAGIC | StartTransaction | Response | A response sent back from the Central System to the Charge Point upon receiving a StartTransaction request. This payload contains a transaction ID. | [example json](https://github.com/Data-Dynamos/excercise-ev-databricks/blob/master/sample-data/StartTransactionResponse.json)
# MAGIC | MeterValues | Request | Message sent at a set frequency (configured per Charge Point) until the Transaction has ended that samples energy throughput at various outlets. Measurand `Energy.Active.Import.Register` gives a cumulative reading of the charge that has been dispensed for the transaction. Measurand `Power.Active.Import` gives the instantaneous charge at the time of reading. This data contains a transaction ID. | [example json](https://github.com/Data-Dynamos/excercise-ev-databricks/blob/master/sample-data/MeterValuesRequest.json) |
# MAGIC | MeterValues | Response | A response sent back from the Central System to the Charge Point upon receiving a MeterValues request. | [example json](https://github.com/Data-Dynamos/excercise-ev-databricks/blob/master/sample-data/MeterValuesResponse.json) |
# MAGIC | StopTransaction | Request | Event sent when the car has stopped a Transaction. It contains a transaction ID, the stop timestamp of the charge, and the meter reading (`meter_stop`) at the time of the event. | [example json](https://github.com/Data-Dynamos/excercise-ev-databricks/blob/master/sample-data/StopTransactionRequest.json) |
# MAGIC | StopTransaction | Response | A response sent back from the Central System to the Charge Point upon receiving a MeterValues request. | [example json](https://github.com/Data-Dynamos/excercise-ev-databricks/blob/master/sample-data/StopTransactionResponse.json) |
# MAGIC
